    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-rose-50 via-purple-50 to-indigo-50 py-8 px-4 sm:px-6 lg:px-8 flex justify-center pb-24">
      ...
      <div className="w-full max-w-3xl space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
             { title: t('hobbies'), icon: HeartIcon, color: "text-rose-500", bg: "bg-rose-50", link: "/onboarding/hobbies" },
             { title: t('locations'), icon: MapPinIcon, color: "text-indigo-500", bg: "bg-indigo-50", link: "/onboarding/locations" },
             { title: t('notifications'), icon: BellIcon, color: "text-amber-500", bg: "bg-amber-50", link: "#notifications" },
             { title: t('privacy') || 'Privacy', icon: ShieldIcon, color: "text-emerald-500", bg: "bg-emerald-50", link: "#privacy" }
          ].map((item, i) => (
            <button 
              key={i}
              onClick={() => item.link ? (item.link.startsWith('#') ? document.getElementById(item.link.slice(1))?.scrollIntoView({ behavior: 'smooth' }) : router.push(item.link)) : null}
        ...
        <div className="space-y-6">
          ...
          <section className="space-y-3">
             <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest ml-4">{t('language')}</h3>
             <Card className="border-0 bg-white/80 backdrop-blur-xl shadow-lg shadow-indigo-50/50 rounded-[2rem]">
                <CardContent className="p-6">
                   <div className="flex items-center justify-between">
                     <div className="flex items-center gap-4">
                       ...
                       <div>
                         <p className="font-semibold text-gray-800">{t('selectLanguage')}</p>
          ...
          <section id="notifications" className="space-y-3">
             <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest ml-4">{t('notifications')}</h3>
             <Card className="border-0 bg-white/80 backdrop-blur-xl shadow-lg shadow-indigo-50/50 rounded-[2rem] overflow-hidden">
                <CardContent className="p-6 divide-y divide-gray-100">
                   <SettingRow 
                     icon={BellIcon} 
                     title={t('pauseNotifications') || 'Pause Notifications'}
                     subtitle={t('pauseNotificationsDesc') || 'Temporarily mute all alerts'}
                     action={<Toggle active={!notificationsEnabled} onClick={() => setNotificationsEnabled(!notificationsEnabled)} />} 
                   />
                   <SettingRow 
                     icon={GlobeIcon} 
                     title={t('emailDigests') || 'Email Digests'}
                     subtitle={t('emailDigestsDesc') || 'Weekly summary of your matches'}
                   ...
                   <SettingRow 
                     ...
                     subtitle={t('pushNotificationsDesc') || 'Real-time alerts on your device'}
           ...
           <section id="privacy" className="space-y-3">
             <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest ml-4">{t('privacy') || 'Privacy'}</h3>
             <Card className="border-0 bg-white/80 backdrop-blur-xl shadow-lg shadow-indigo-50/50 rounded-[2rem] overflow-hidden">
                <CardContent className="p-6 divide-y divide-gray-100">
                   <SettingRow 
                     icon={LockIcon} 
                     title={t('privateProfile') || 'Private Profile'}
                     subtitle={t('privateProfileDesc') || 'Hide details from non-matches'}
                     action={<Toggle active={privateProfile} onClick={() => setPrivateProfile(!privateProfile)} />} 
                   />
                   <SettingRow 
                     icon={MapPinIcon} 
                     title={t('showDistance') || 'Show Distance'}
                     subtitle={t('showDistanceDesc') || 'Visible to people nearby'}
