...
import { Card, CardContent, Badge, Button, Avatar, AvatarFallback, AvatarImage } from '@/components/ui';
import { MOCK_EVENTS } from '@/mock-data';
import { getHobbyById, getLocationById, getUserById } from '@/lib/data';

// Inline Icons for better styling without external dependencies
      ...
      <div className="max-w-7xl mx-auto space-y-10">
              ...
              >
                ...
                <div className="h-36 bg-gradient-to-br from-rose-400 via-pink-500 to-purple-600 relative overflow-hidden shrink-0">
                  ...
                  <div className="absolute bottom-4 left-6 text-white flex items-center gap-3 drop-shadow-md">
                     <div className="text-3xl filter drop-shadow-lg">{hobby?.icon}</div>
                     <span className="font-bold text-lg tracking-tight">{hobby?.name}</span>
                  </div>
                </div>
                ...
                <CardContent className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="pt-4 border-t border-gray-100/50 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      <Avatar className="w-9 h-9 ring-2 ring-white shadow-sm border border-rose-100">
                        <AvatarImage src={host?.image || ''} alt={host?.name || ''} />
                        <AvatarFallback className="bg-gradient-to-br from-rose-100 to-pink-100 text-rose-600 text-xs font-bold">
                          {host?.name?.charAt(0) || '?'}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Host</span>
                        <span className="text-xs font-bold text-gray-700 truncate max-w-[90px]">{host?.name}</span>
                      </div>
                    </div>

                    <div className="flex -space-x-2.5 items-center">
                      {[1, 2, 3].map((i) => (
                        <div key={i} className="w-7 h-7 rounded-full bg-gradient-to-tr from-pink-200 to-purple-200 ring-2 ring-white shadow-sm"></div>
                      ))}
                      ...
                      <span className="ml-3 text-xs font-semibold text-rose-400">
                        {event.currentParticipants.length} joined
                      </span>
                    </div>
                  </div>
                </CardContent>
        ...
        {openEvents.length === 0 && (
           <div className="flex flex-col items-center justify-center py-24 px-4 text-center animate-in fade-in zoom-in duration-500">
              ...
              <h3 className="text-2xl font-black text-gray-800 mb-2 tracking-tight">No Vibes Found?</h3>
              <p className="text-gray-500 mb-8 max-w-md mx-auto text-lg">
                 Looks like its quiet right now. Be the spark and start your own meetup!
              </p>
              <Button 
