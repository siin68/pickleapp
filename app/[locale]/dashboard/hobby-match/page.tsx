...
const FilterIcon = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
);
...
const XIcon = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
);
...
const InfoIcon = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
);
...
const LocationIcon = ({ className }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
);
...
export default function HobbyMatchPage() {
  ...
  const filteredUsers = MOCK_USERS.slice(1, 10).filter(user => {
    // Age filter
    if (user.age && (user.age < ageRange.min || user.age > ageRange.max)) {
      return false;
    }
    
    // Hobby filter (if any hobbies selected, user must have at least one matching hobby)
    if (selectedHobbies.length > 0) {
      const hasMatchingHobby = user.hobbies.some(hobbyId => selectedHobbies.includes(hobbyId));
      if (!hasMatchingHobby) {
        return false;
      }
    }
    
    // Distance filter (mock: assuming distance is random 1-100km)
    // In real app, calculate actual distance based on user location
  ...
  // Button handlers
  const onBtnSwipe = (dir: 'left' | 'right') => {
    triggerSwipe(dir);
  };
  ...
  const activeFiltersCount = selectedHobbies.length + 
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center px-3 py-2 sm:py-4 relative overflow-hidden select-none">
      ...
      <div className="text-center mb-3 sm:mb-6 relative z-10">
        ...
        <p className="text-gray-500 text-[11px] sm:text-xs md:text-sm font-medium">Swipe right to like, left to pass</p>
      </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-gray-600 mb-1 block">Min Age: {ageRange.min}</label>
                    <input
                      type="range"
                      min="18"
                      max="50"
                  ...
                  <div>
                    <label className="text-xs text-gray-600 mb-1 block">Max Age: {ageRange.max}</label>
                    <input
                      type="range"
                      min="18"
                      max="50"
                ...
                <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                  {HOBBIES.map((hobby) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold truncate">{hobby.name}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {nextUser && (
           <div className="absolute inset-0 top-3 sm:top-4 scale-95 opacity-50 bg-white rounded-[1.75rem] sm:rounded-[2rem] border border-gray-200 shadow-xl overflow-hidden pointer-events-none">
              <div className="w-full h-full bg-gray-300">
                <img src={nextUser.image} className="w-full h-full object-cover grayscale" alt="" />
              </div>
           </div>
        )}
        ...
        {currentUser ? (
          <div 
            ...
            className={`
              ...
              ${flyAway === 'right' ? 'translate-x-[200%] rotate-12 opacity-0 transition-all duration-500 ease-out' : ''}
              ${flyAway === 'left' ? '-translate-x-[200%] -rotate-12 opacity-0 transition-all duration-500 ease-out' : ''}
              ${!isDragging && !flyAway ? 'transition-transform duration-300 cubic-bezier(0.175, 0.885, 0.32, 1.275)' : ''}
            `}
            style={{
              transform: !flyAway ? `translate(${xDelta}px, ${currentPos.y - startPos.y}px) rotate(${rotate}deg)` : undefined,
          ...
          >
            ...
            <div className="absolute inset-0 bg-gray-200 pointer-events-none">
               <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-500">
                 <img src={currentUser.image} alt={currentUser.name} className="w-full h-full object-cover pointer-events-none" draggable={false} />
               </div>
            ...
            <div className="absolute bottom-0 inset-x-0 p-3 sm:p-4 md:p-6 text-white pointer-events-none z-10">
               <div className="flex items-end justify-between mb-1.5 sm:mb-2 md:mb-3">
                  <div>
                    <h2 className="text-2xl sm:text-3xl md:text-4xl font-black leading-none drop-shadow-md">
                      {currentUser.name} 
                      <span className="text-lg sm:text-xl md:text-2xl font-medium opacity-80 ml-1.5 sm:ml-2">{currentUser.age}</span>
                    </h2>
               ...
               <div className="flex flex-wrap gap-1.5 sm:gap-2">
                 {currentUser.hobbies.slice(0, 3).map((hobbyId) => {
                    const hobby = getHobbyById(hobbyId);
                    return (
                      <span key={hobbyId} className="bg-white/20 backdrop-blur-md px-2 sm:px-2.5 md:px-3 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] md:text-xs font-bold shadow-sm border border-white/20">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 sm:p-6 md:p-8 bg-white/60 backdrop-blur-xl rounded-[1.75rem] sm:rounded-[2rem] md:rounded-[2.5rem] border-2 border-dashed border-gray-300 shadow-xl">
             ...
             <h3 className="text-lg sm:text-xl md:text-2xl font-black text-gray-800 mb-2">No more profiles!</h3>
             <p className="text-xs sm:text-sm md:text-base text-gray-500 mb-5 sm:mb-6 md:mb-8 max-w-[160px] sm:max-w-[180px] md:max-w-[200px]">Youve seen everyone nearby. Check back later.</p>
             <Button 
               onClick={resetDeck} 
